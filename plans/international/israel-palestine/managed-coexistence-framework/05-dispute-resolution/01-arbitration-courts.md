# Confederal Arbitration Courts

**Parent Document:** [00-introduction.md](../00-introduction.md#dispute-resolution-mechanics)

**Phase:** All Phases

**Status:** Final Draft

---

## Purpose

Confederal Arbitration Courts provide **neutral, predictable dispute resolution** for conflicts between individuals participating in MCF programs.

Arbitration does not replace national courts. It offers a venue where cross-jurisdictional disputes would otherwise be unresolvable—where different legal systems, different enforcement mechanisms, and different political loyalties make national courts inadequate for disputes crossing community boundaries.

---

## Arbitration Principles

### 1. Civil, Non-Criminal Jurisdiction

Arbitration courts handle civil matters only:
- Commercial disputes
- Contractual conflicts
- Labor disagreements
- Property disputes
- CMS-related conduct issues

Criminal matters remain outside MCF jurisdiction.

### 1.1 Relationship to Commercial Courts

**Forum Priority:** For commercial disputes and labor disputes within [Confederal Commercial Court](../07-economic-architecture/03-commercial-courts.md) jurisdiction, Commercial Courts are the primary forum unless parties explicitly agree to arbitration.

**When Arbitration Applies:**

| Scenario | Forum |
|----------|-------|
| Parties' contract specifies arbitration | Arbitration |
| Parties' contract specifies Commercial Court or is silent | Commercial Court |
| Dispute involves MCF-registered employer labor claim | Commercial Court (primary) |
| Dispute involves CMS-related conduct or credentials | Arbitration (unless CMS revocation—see Review Panels) |
| Property disputes between participants | Arbitration |
| Parties mutually agree to arbitrate after dispute arises | Arbitration |

**Rationale:** Commercial Courts provide full adjudicative procedures with appellate review and detailed choice-of-law rules. Arbitration provides flexibility for parties who prefer it, and serves as the forum for dispute categories outside Commercial Court scope.

### 2. Voluntary, Benefit-Linked Participation

Arbitration access is linked to CMS membership or program enrollment, but use of arbitration requires explicit agreement:

- CMS membership provides *eligibility* for arbitration, not automatic submission
- For commercial and labor disputes within [Commercial Court](../07-economic-architecture/03-commercial-courts.md) jurisdiction, Commercial Courts are primary unless parties explicitly agree to arbitrate (per Section 1.1)
- Explicit arbitration agreement means: contract clause specifying arbitration, or mutual post-dispute agreement to arbitrate
- CMS enrollment alone does not constitute an arbitration agreement for commercial/labor matters

This preserves CCC primacy while ensuring arbitration remains available for disputes outside Commercial Court scope or where parties prefer it.

### 3. Procedural Neutrality

Arbitration procedures are:
- Standardized across all cases
- Anchored in due process principles
- Transparent in evidence handling
- Equal in access regardless of community

No procedural advantage for either party based on identity.

### 4. Limited Enforcement

Enforcement is limited to MCF benefit scope:

- Contract enforcement within Framework
- Program access revocation
- Benefit withholding
- CMS credential actions (suspension, notation, revocation) → referred to CMS Review Panel; arbitration recommends, Review Panel decides

Arbitration cannot enforce beyond MCF's own authority.

---

## Court Structure

### 1. Arbitration Panels

**Composition:**
- Three arbitrators per panel
- Community balance (one from each major community, one neutral/international)
- Professional qualifications required
- Conflict-of-interest screening

**Selection:**
- Arbitrators drawn from qualified pool
- Parties may strike limited number
- Random assignment from remaining pool
- Presiding arbitrator selected by panel or rotation

### 2. Arbitrator Qualifications

**Required:**
- Legal training (law degree or equivalent)
- 5+ years relevant professional experience
- Arbitration certification (recognized program)
- Language proficiency (Arabic, Hebrew, English)
- No disqualifying conflicts

**Preferred:**
- Commercial law expertise
- Cross-cultural dispute resolution experience
- International arbitration experience
- Technical expertise relevant to dispute types

### 3. Arbitrator Pool Management

**Pool Composition:**
- Minimum 30 qualified arbitrators
- Community balance maintained
- Geographic distribution
- Specialty coverage

**Pool Administration:**
- Administered by MCF Secretariat
- Annual qualification review
- Performance evaluation
- Removal for cause

### 4. Administrative Support

**Secretariat Functions:**
- Case intake and registration
- Panel assignment
- Scheduling and logistics
- Record keeping
- Decision publication
- Fee administration

**Staffing:**
- Registrar (legal professional)
- Case managers
- Administrative support
- Translation services

---

## Procedures

### 1. Filing and Intake

**Filing Requirements:**
- Written complaint specifying:
  - Parties involved
  - Nature of dispute
  - Relief sought
  - Basis for arbitration jurisdiction
  - Relevant documentation
- Filing fee (means-tested)
- Confirmation of arbitration agreement

**Intake Process:**
- Jurisdictional screening
- Respondent notification
- Response deadline (30 days)
- Panel assignment

### 2. Pre-Hearing Procedures

**Case Management Conference:**
- Issues identification
- Document exchange schedule
- Witness identification
- Hearing scheduling
- Procedural agreements

**Discovery:**
- Limited document exchange
- Written interrogatories (limited)
- No depositions (cost control)
- Privilege protections

**Motions:**
- Jurisdictional challenges
- Dismissal motions
- Interim measures requests

### 3. Hearing Procedures

**Hearing Format:**
- In-person preferred, remote permitted
- Interpretation services provided
- Evidentiary standards specified
- Witness examination
- Party presentations
- Closing arguments

**Evidentiary Standards:**
- Relevance requirement
- Authentication standards
- Hearsay limitations (flexible)
- Expert testimony procedures
- Document admissibility

**Record:**
- Full transcription
- Recording (audio/video)
- Exhibit preservation
- Confidentiality maintained

### 4. Decision and Award

**Decision Timeline:**
- Award within 60 days of hearing close
- Extensions by panel decision (limited)

**Award Content:**
- Findings of fact
- Legal conclusions
- Relief awarded
- Costs allocation
- Reasoning explanation

**Award Form:**
- Written, signed by panel majority
- Dissent permitted
- Published (redacted for privacy)

### 5. Appeal Rights

**Grounds for Appeal:**
- Procedural irregularity affecting outcome
- Arbitrator misconduct or bias
- Award exceeds jurisdiction
- Manifest error of law

**Appeal Process:**
- Filed within 30 days
- Appellate panel (different arbitrators)
- Record review, not de novo
- Affirm, reverse, or remand

**Finality:**
- Appellate decision final within MCF
- No further internal appeal

---

## Choice of Law

### 1. Applicable Law Hierarchy

Arbitration adopts the same choice-of-law hierarchy as [Confederal Commercial Courts](../07-economic-architecture/03-commercial-courts.md#5-choice-of-law):

| Priority | Source | Application |
|----------|--------|-------------|
| **1** | MCF Commercial Code | Where MCF has specific rules |
| **2** | Party agreement | Contract choice of law clause |
| **3** | Closest connection | Law of jurisdiction with closest connection |
| **4** | International commercial law | CISG, UNIDROIT Principles as gap-filler |

### 2. Application in Arbitration

**Party Autonomy:** Parties may specify applicable law in their arbitration agreement. Absent such specification, arbitrators apply the hierarchy above.

**MCF Rules Primary:** For disputes arising from MCF programs (CMS, SAZ, labor mobility), MCF rules apply as primary source regardless of party choice.

**Legal Conclusions in Awards:** Awards that include legal conclusions (per Procedures Section 4, "Decision and Award") must identify the applicable law applied. Appeals for "manifest error of law" are assessed against the correct source in the hierarchy.

---

## Subject Matter Jurisdiction

### 1. Commercial Disputes

**Included:**
- Contract disputes between CMS participants
- Business-to-business conflicts
- Service agreements
- Licensing disputes
- Partnership disagreements

**Requirements:**
- Both parties CMS participants or program enrollees
- Arbitration agreement (in contract or program terms)
- Commercial character of dispute

### 2. Labor Disputes

**Included:**
- Employment contract disputes
- Wage and benefit claims
- Wrongful termination (employment aspect only)
- Workplace condition disputes
- Labor agreement interpretation

**Limitations:**
- Individual disputes only (not collective bargaining)
- Employment relationship, not union matters
- Civil remedies only (no criminal sanctions)

### 3. Property Disputes

**Included:**
- Movable property disputes
- Commercial lease disputes
- Intellectual property (within MCF programs)
- Damage claims

**Excluded:**
- Real property ownership (sovereignty implications)
- Land title disputes
- Territorial claims
- Religious property

### 4. CMS-Related Conduct

**Included:**
- Disputes arising from CMS participation
- Benefit access conflicts
- Credential recognition disputes
- Program eligibility disagreements

**Limitations:**
- Between participants (not authority disputes)
- CMS revocation remains with CMS Review Panel under [01-cms-framework.md](../02-phase-1-civic-membership/01-cms-framework.md)
- Cannot override CMS administrative decisions
- Arbitration awards recommending CMS credential action (suspension, notation, revocation) are referred to CMS Review Panel for implementation—arbitration recommends, Review Panel decides

---

## Enforcement

### 1. Framework-Internal Enforcement

**Enforcement Tools:**
- Withhold Framework benefits pending compliance
- Program access revocation
- Guarantor-supported pressure

**CMS-Related Enforcement (referral required):**

- Credential suspension → referred to CMS Review Panel
- CMS standing notation → referred to CMS Review Panel
- CMS revocation recommendation → referred to CMS Review Panel

Arbitration may recommend CMS credential actions as part of an award, but implementation authority rests with [CMS Review Panel](02-review-panels.md). Review Panel considers arbitration award as binding on facts but retains discretion on proportionality of credential action.

**Enforcement Process:**
- Award becomes enforceable after appeal period
- Non-compliance documented
- Notice to non-compliant party
- Enforcement measures activated
- Compliance restoration upon satisfaction

### 2. Recognition Agreements

**Domestic Court Recognition:**
- Pursue recognition agreements with both jurisdictions
- Awards treated as enforceable judgments where recognized
- Enforcement through domestic court mechanisms

**Recognition Limitations:**
- Depends on domestic court cooperation
- May face political obstacles
- Alternative: Framework-internal enforcement

**International Recognition:**
- New York Convention applicability explored
- International arbitration standards followed
- Cross-border enforcement where possible

### 3. Voluntary Compliance Incentives

**Compliance Benefits:**
- Maintained CMS standing
- Continued program access
- Good-faith notation in records
- Future dispute resolution access

**Non-Compliance Consequences:**
- CMS standing notation
- Benefit withholding
- Program exclusion
- Reputational effect

---

## Fees and Costs

### 1. Filing Fees

**Fee Structure:**
- Base filing fee: $100-500 (scaled by claim value)
- Expedited processing: Additional fee
- Small claims track: Reduced fee

**Means Testing:**
- Fee waiver for demonstrated need
- Payment plans available
- No denial of access for inability to pay

### 2. Arbitrator Fees

**Fee Calculation:**
- Daily rate for hearing days
- Preparation time (capped)
- Decision drafting time

**Fee Sharing:**
- Default: Equal split
- Award may reallocate based on outcome
- Prevailing party recovery possible

### 3. Administrative Costs

**Covered by MCF:**
- Secretariat operations
- Facilities
- Basic interpretation
- Record keeping

**Party-Paid:**
- Expert witnesses
- Special interpretation
- Expedited processing
- Document production beyond basic

### 4. Cost Recovery

**In Award:**
- Arbitrators may allocate costs
- Factors: Outcome, party conduct, resources
- No automatic prevailing party rule

**Limits:**
- No costs creating access barrier
- Proportionality to claim value
- Hardship considerations

---

## Quality Assurance

### 1. Arbitrator Evaluation

**Performance Metrics:**
- Timeliness of decisions
- Procedural compliance
- Party feedback
- Appeal reversal rate
- Efficiency measures

**Evaluation Process:**
- Annual review
- Peer evaluation
- Party satisfaction surveys
- Outcome analysis

### 2. Procedural Audits

**Audit Scope:**
- Procedural compliance
- Documentation completeness
- Timeliness adherence
- Fee handling
- Record integrity

**Audit Frequency:**
- Annual comprehensive audit
- Random case audits
- Complaint-triggered reviews

### 3. Continuous Improvement

**Improvement Mechanisms:**
- Procedure updates based on experience
- Arbitrator training programs
- Technology improvements
- User feedback integration

---

## Special Procedures

### 1. Small Claims Track

**Eligibility:**
- Claims under $5,000
- Simple factual disputes
- Limited parties

**Simplified Procedures:**
- Single arbitrator
- Document-only option
- Abbreviated timeline
- Reduced fees

### 2. Expedited Procedures

**Eligibility:**
- Urgent matters
- Perishable goods
- Time-sensitive contracts
- Party agreement

**Accelerated Timeline:**
- Hearing within 30 days
- Decision within 15 days of hearing
- Limited discovery
- Additional fees

### 3. Interim Measures

**Available Measures:**
- Asset preservation
- Status quo maintenance
- Harm prevention
- Evidence preservation

**Interim Measure Process:**
- Emergency application
- Panel or emergency arbitrator
- Expedited consideration
- Reviewable at hearing

---

## Confidentiality

### 1. Proceeding Confidentiality

**Confidential:**
- Hearing proceedings
- Submitted documents
- Witness testimony
- Deliberations

**Not Confidential:**
- Existence of proceeding (unless agreed)
- Final award (redacted for publication)
- Precedential holdings

### 2. Party Privacy

**Protected:**
- Personal information
- Business confidential information
- Health information
- Financial details

**Disclosure Permitted:**
- To enforcement authorities
- To guarantors when relevant
- For accountability purposes
- With party consent

### 3. Publication

**Published (Redacted):**
- Decision summaries
- Procedural rulings
- Precedential interpretations

**Purpose:**
- Transparency
- Precedent development
- Public accountability
- System credibility

---

## References

- [Dispute Resolution Overview](00-overview.md)
- [Review Panels](02-review-panels.md)
- [Jurisdictional Boundaries](04-jurisdictional-boundaries.md)
- [Civic Membership System](../02-phase-1-civic-membership/01-cms-framework.md)
